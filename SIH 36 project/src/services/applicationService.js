...
export const applicationService = {
  ...
  async updateStatus(id, newStatus, remarks = '') {
    try {
      // If decision is APPROVED or REJECTED, call official officer endpoint
      const decision = newStatus === 'APPROVED' ? 'APPROVED' : (newStatus === 'REJECTED' ? 'REJECTED' : null);
      if (decision) {
        const res = await apiClient.post(`/officer/applications/${id}/decision`, {
          decision,
          remarks: remarks || `Officer review concluded: ${decision}`
        });
        ...
        if (idx !== -1) {
          localApplications[idx] = {
            ...localApplications[idx],
            status: decision === 'APPROVED' ? 'OFFICER_APPROVED' : 'OFFICER_REJECTED',
