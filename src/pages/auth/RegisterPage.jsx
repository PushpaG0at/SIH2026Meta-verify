...
export const RegisterPage = () => {
  ...
  const handleRoleSelect = (selectedRole) => {
    const upper = (selectedRole || 'BUSINESS').toUpperCase();
  ...
  useEffect(() => {
    ...
    if (roleParam) {
      ...
      if (['BUSINESS', 'INSPECTOR', 'OFFICER'].includes(upper)) {
        handleRoleSelect(upper);
      }
    }
  }, [searchParams]);
  ...
  const validate = () => {
    ...
    if (!formData.password) {
      ...
      errs.password = 'Password must meet the required security rules (minimum 6 characters)';
    }
    ...
    if (!formData.role || !['BUSINESS', 'INSPECTOR', 'OFFICER'].includes(formData.role.toUpperCase())) {
      errs.role = 'Please select a valid stakeholder role';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const assignedRole = (res.role || res.user?.role || formData.role).toUpperCase();

      toast?.success?.(
        `Account registered successfully! Redirecting to ${assignedRole} portal...`
      );

      // Strict role-based redirect
      if (assignedRole === 'BUSINESS') {
        navigate('/business/dashboard');
      } else if (assignedRole === 'INSPECTOR') {
      ...
      if (errMsg.toLowerCase().includes('already exists')) {
        setRegError('An account with this email already exists. Please sign in instead.');
      } else if (errMsg.toLowerCase().includes('unavailable')) {
        setRegError('Registration service is currently unavailable. Please try again.');
      } else if (errMsg.toLowerCase().includes('unable to connect') || errMsg.toLowerCase().includes('network')) {
        setRegError('Unable to connect to the registration service.');
      } else if (errMsg.toLowerCase().includes('went wrong') || errMsg.toLowerCase().includes('server error')) {
        setRegError('Something went wrong while creating your account.');
      } else {
        setRegError(errMsg || 'Registration failed. Please review your details and try again.');
      }
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4" noValidate>
        ...
        >
          Register as {formData.role.charAt(0).toUpperCase() + formData.role.slice(1).toLowerCase()} & Continue
        </Button>
      </form>
